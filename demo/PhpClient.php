...
try {
  if (array_search('--http', $argv)) {
    $socket = new THttpClient('thrift.demo.com', 80, '/PhpServer.php');
  } else {
    // $socket = new TSocket('localhost', 9090);
  }
  $transport = new TBufferedTransport($socket, 0, 0);
  $protocol = new TBinaryProtocol($transport, true, true);
  $client = new \tutorial\CalculatorClient($protocol, $protocol);
